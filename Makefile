...
contract:
	...
	mkdir -p res
	cp target/wasm32-unknown-unknown/release/near_guestbook.wasm res/near_guestbook.wasm

.PHONY: deploy-contract # deploy contract
deploy-contract: contract
	near deploy --wasmFile res/near_guestbook.wasm --accountId nearguestbook.akagi201.testnet
